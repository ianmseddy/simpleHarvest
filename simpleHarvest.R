...
defineModule(sim, list(
  ...
  inputObjects = bind_rows(
    ...
    expectsInput("harvestAreas", objectClass = "SpatialPolygonsDataFrame",
                 desc = paste('A shapefile with layers representing unique harvest areas that contains (at least) 3 *IMPORTANT* attributes:',
                              ...
                              'Defaults to TSAs in northern BC'),
                 sourceURL = 'https://drive.google.com/open?id=1RrmxeGJB98lgsVnl-TiKC1lJ7pwIX0ls'),
...
.inputObjects <- function(sim) {
  ...
  if (!suppliedElsewhere("harvestAreas", sim)) {
    ...
    browser()
    harvestAreas <- prepInputs(url = extractURL('harvestAreas', sim),
                               studyArea = sim$studyArea,
                               rasterToMatch = sim$rasterToMatch,
                               destinationPath = dPath,
                               cacheTags = c(cacheTags, "harvestAreas"),
                               useCache = TRUE)

    harvestAreas$AAC <- as.numeric(harvestAreas$AAC) #WHY THE HELL IS THIS A CHARACTER?
    harvestAreas$AnnualBiomassHarvestTarget <- harvestAreas$AAC * 0.45
    #cubic metres x average Mg/m3 of wood https://cedarstripkayak.wordpress.com/lumber-selection/162-2/
  ...
  if (!suppliedElsewhere("areasToExclude", sim)) {
    message("no default areas to exclude - will use BC parks")

    rivers <- prepInputs(url = 'https://drive.google.com/open?id=1UB6_uOIK6_Uyn8ZWmUE6w0vikdnPyjr5',
                         studyArea = sim$studyArea,
                         rasterToMatch = sim$rasterToMatch,
                         destinationPath = dPath,
                         fun = 'shapefile',
                         useCache = TRUE,
                         cacheTags = c(cacheTags, "rivers"))
    parks <- prepInputs(url = 'https://drive.google.com/open?id=1BzpXy6Bpoy4QFpqcichhXFAg2tuUTYL0',
                        studyArea = sim$studyArea,
                        rasterToMatch = sim$rasterToMatch,
                        destinationPath = dPath,
                        fun = 'shapefile',
                        useCache = TRUE,
                        cacheTags = c(cacheTags, 'parks'))

    #at the moment prepInputs is not sourcing the .prj correctly when using st_read
    parks <- sf::st_as_sf(parks)
    rivers <- sf::st_as_sf(rivers)
    rivers <- sf::st_buffer(rivers, 500) #assumes m
    protAreas <- sf::st_union(parks, rivers) %>%
      sf::as_Spatial(.)
    sim$areasToExclude <- protAreas
  }

  if (!suppliedElsewhere('DEMraster', sim)) {

    sim$DEMraster <- prepInputs(url = 'https://drive.google.com/open?id=1DmP4LKkb7EyHgUKCaM6PVJdLPwkInODD',
                                rasterToMatch = sim$rasterToMatch,
                                studyArea = sim$studyArea,
                                targetFile = 'Hydro1km_Canada.tif',
                                destinationPath = dPath,
                                useCache = TRUE,
                                cacheTags = c(cacheTags, 'dem'))
  }
