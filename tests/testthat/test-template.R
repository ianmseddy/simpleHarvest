...
#    `functionName` is a function's name in your module (e.g., `simpleHarvestEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/ieddy/Git/simpleHarvest/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("simpleHarvest")
  path <- list(modulePath = "/home/ieddy/Git",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    simpleHarvest = list(.saveInitialTime = NA)
  )
  ...
  if (exists("simpleHarvestEvent1", envir = .GlobalEnv)) {
    simOutput <- simpleHarvestEvent1(mySim)
  } else {
  ...
  if (exists("simpleHarvestEvent2", envir = .GlobalEnv)) {
    simOutput <- simpleHarvestEvent2(mySim)
  } else {
